...
from pyfx.ui.new_pedal_config_dialog_ui import Ui_NewPedalConfigDialog
...
class NewPedalConfigDialog(QDialog, Ui_NewPedalConfigDialog):
    ...
    def accept(self):
        ...
        knobs = {}
        for knob_idx in range(knob_cnt):
            name = f"Knob {knob_idx}"
            knobs[name] = KnobConfig(name=name)

        footswitches = {}
        for footswitch_idx in range(switch_cnt):
            name = f"Footswitch {footswitch_idx}"
            footswitches[name] = FootswitchConfig(name=name)
